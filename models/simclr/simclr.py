...
class ResNetSimCLR(nn.Module):
    """SimCLR model"""
    def __init__(self, dataset, base_model, out_dim):
        super(ResNetSimCLR, self).__init__()
        self.resnet_dict = {"resnet18": RN.ResNet(dataset, 18, out_dim),
                            "resnet32": RN.ResNet(dataset, 32, out_dim),
                            "resnet50": RN.ResNet(dataset, 50, out_dim)}
        ...
        for epoch in range(self.args.epochs):
            for images, _ in tqdm(train_loader):
                ...
                with autocast(enabled=self.args.mixed_precision):
                    ...
                    logits, labels = self.criterion.log, self.criterion.la
